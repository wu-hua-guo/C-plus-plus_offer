...
void Print(BinaryTreeNode* pRoot)
{
    ...
    std::stack<BinaryTreeNode*> stackData[2];
    int current = 0;
    int next = 1;
    stackData[current].push(pRoot);
    while(!stackData[current].empty()||!stackData[next].empty())
    {
        BinaryTreeNode* pNode = stackData[current].top();
        stackData[current].pop();
        cout << pNode->m_nValue <<" ";

        if(current == 0)
        {
            if(pNode->m_pLeft!=nullptr)
                stackData[next].push(pNode->m_pLeft);
            if(pNode->m_pRight!=nullptr)
                stackData[next].push(pNode->m_pRight);
