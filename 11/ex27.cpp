...
void MirrorRecursively_1(BinaryTreeNode *pNode)
{
...
void printTreeNodes(BinaryTreeNode* pRoot)
{
    if(pRoot == nullptr)
        return;
    std::queue<BinaryTreeNode*> stackTreeNode;
    stackTreeNode.push(pRoot);

    while(stackTreeNode.size() > 0)
    {
        BinaryTreeNode* pNode = stackTreeNode.front();
        cout << pNode->m_nValue <<" ";
        stackTreeNode.pop();
...
void MirrorIteratively_1(BinaryTreeNode* pRoot)
{
    cout << "stack:"<<endl;
    if(pRoot == nullptr)
        return;
